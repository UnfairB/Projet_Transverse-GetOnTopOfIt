...
class Javelin(pg.sprite.Sprite):
    ...
    def __init__(self, game_state, player, target_pos_world):
        """
        Initialiser le javelot
        """
        super().__init__()
        self.game_state = game_state
        self.player = player  # Propriétaire du javelot

        # Charger l'image du javelot
        try:
            ...
            self.original_image = pg.Surface((TILE_SIZE, TILE_SIZE // 4))
            self.original_image.fill((100, 100, 100))

        self.image = self.original_image.copy()
        self.rect = self.image.get_rect()

        # Calculer de la direction initiale
        direction_vector = pg.math.Vector2(target_pos_world) - pg.math.Vector2(player.rect.center)
        if direction_vector.length() == 0:  # Éviter la division par zéro si la souris est sur le joueur
            direction_vector = pg.math.Vector2(1, 0)  # Par défaut, tire vers la droite

        # Position initiale du javelot
        ...
        self.rotate()  # Orienter le javelot initialement

    def rotate(self):
        """
        Orienter l'image du javelot en fonction de sa vélocité (s'il vole) ou de son angle lorsqu'il est planté
        """
        if self.state == 'flying':
            # Calculer l'angle basé sur la vélocité. atan2 prend (y, x)
            # Négatif sur Y car les coordonnées Pygame Y augmentent vers le bas
            angle_rad = math.atan2(-self.vel.y, self.vel.x)
            self.stuck_angle = math.degrees(angle_rad) # Mettre à jour l'angle pour le cas où il se plante
    ...
    def update(self, dt): # dt (delta time) n'est pas utilisé ici pour rester simple, mais serait utile pour des FPS variables
        """ Mettre à jour la position et l'état du javelot. """
        if self.state == 'flying':
            ...
            # Détecter la collision de la lance avec le zombie
            if hasattr(self.game_state, "monsters"):
                for monster in self.game_state.monsters:
                    if not getattr(monster, "is_dead", False) and self.rect.colliderect(monster.rect):
                        # Animer la mort du zombie
                        # Charger les sprites de mort
                        try:
                            death_imgs = [
            ...
            # Le javelot ne bouge plus et ne tourne plus une fois planté. Sa position et son angle sont figés
            pass
            ...
            if direction_to_player.length_squared() < (JAVELIN_RECALL_SPEED * JAVELIN_RECALL_SPEED /4) : # Proche du joueur
                self.player.retrieve_javelin() # Notifier le joueur
            ...
            self.pos += self.vel
            self.rect.center = self.pos
            self.rotate() # S'orienter vers le joueur pendant le retour
    ...
    def check_collision_walls(self):
        ...
        if collided_platforms:
            ...
            # Ajouter ce javelot au groupe des plateformes
            self.game_state.platforms.add(self)

            # Vérifier si le joueur chevauche le javelot et ajuste sa position
